...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Delay the animations slightly to ensure DOM elements are ready
    ...
    {loading ? (
      ...
      >
        ...
        <div className={`h-1/5 md:h-3/5 flex flex-col w-full text-center ${choiceTheme == 'red' ?  'text-red-200' : choiceTheme == 'blue' ? 'text-blue-200' : choiceTheme == 'white' ? 'text-zinc-400' : 'text-gray-200'}`}>
          <div
            ref={titleRef}
            className={`h-1/4 overflow-hidden backdrop-hue-rotate-90 backdrop-grayscale  backdrop-contrast-150 backdrop-blur-md backdrop-brightness-110 rounded-b-xl ml-4 mr-4 ${choiceTheme == 'red' ?  'bg-red-600 text-zinc-900' : choiceTheme == 'blue' ? 'bg-blue-700 text-zinc-50' : choiceTheme == 'white' ? 'bg-zinc-300 text-zinc-800' : 'bg-black'}`}
          >
            {/* <div className='font-jersey text-6xl'>HELLO myself Sarang</div>
            <p className='font-jersey text-3xl'> Full stack Web Developer</p> */}
            {/* <img className='w-14' src="./pixA2.png" alt="" /> */}
          ...
          <div className="flex flex-col md:flex md:flex-row h-3/4 m-4 rounded-lg backdrop-hue-rotate-90 backdrop-grayscale  backdrop-contrast-150 backdrop-blur-md backdrop-brightness-110">
            <div className="relative h-2/4 md:w-2/3">
              <img
                className={`cursor-none size-56 border-4 hover:border-transparent border-spacing-9 ${timeTheme === 'morning' ? 'border-blue-200' : timeTheme === 'afternoon' ? 'border-orange-200' : 'border-zinc-200'} md:size-auto relative`}
                src="https://i.pinimg.com/564x/45/a9/3c/45a93c4d076a5c19169f9f389f8ee45f.jpg"
                alt=""
              />
            </div>
        ...
        <div className={`h-2/5 text-gray-200 m-2 rounded-lg  font-pixelify-sans text-2xl ${choiceTheme == 'red' ?  'bg-black' : choiceTheme == 'blue' ? 'bg-black' : choiceTheme == 'white' ? 'bg-zinc-300 text-gray-900' : 'bg-stone-900 text-gray-200'}`}>
          <div className="m-4 ">
            <div>
              Projects
            </div>
            <div ref={projectsRef} className='flex md:flex-row items-center flex-col overflow-hidden md:overflow-x-auto custom-scrollbar text-zinc-900 mb-3 pb-5'>
